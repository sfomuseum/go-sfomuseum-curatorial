...
func main() {
	...
	iterator_uri := flag.String("iterator-uri", "repo://?include=properties.sfomuseum:placetype=boardingarea&exclude=properties.edtf:deprecated=.*", "...")
	...
	outdir := flag.String("outdir", "/usr/local/data/sfomuseum-data-architecture/sfomuseum/boardingareas", "...")
	...
	iter_cb := func(ctx context.Context, path string, r io.ReadSeeker, args ...interface{}) error {
		...
		rsp := gjson.GetBytes(body, "properties.sfomuseum:boardingarea_id")
	...
	lookup.Range(func(k interface{}, v interface{}) bool {
		boardingareas = append(boardingareas, k.(string))
		return true
	...
	for _, boardingarea_id := range boardingareas {
		...
		q := u.Query()
		q.Set("include", fmt.Sprintf("include=properties.sfomuseum:boardingarea_id=^%s$", boardingarea_id))
		...
		opts := &render.RenderOptions{
			...
			IteratorURI:     boardingarea_iterator_uri,
